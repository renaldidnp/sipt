...
class Jadwal extends CI_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model('JadwalModel'); // Memuat model yang sudah dibuat sebelumnya
    }

    public function index()
    {
        $data['jadwal_acara'] = $this->JadwalModel->get_jadwal_acara();
    ...
    {
        $data['jadwal_acara'] = $this->PembayaranModel->get_pembayaran_by_id($id);
        $this->load->view('template/header');
    ...
    public function search()
    {
        ...
        if (!empty($keyword)) {
            ...
            $data['jadwal_acara'] = $this->PembayaranModel->cari_pembayaran($keyword);
        } else {
            // Jika $keyword kosong, tampilkan semua data pendaftaran
            $data['jadwal_acara'] = $this->PembayaranModel->get_pembayaran();
        }
